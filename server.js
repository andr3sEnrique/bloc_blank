...
app.get("/api/vehicules", (req, _res, next) => {
  ...
  const sql = 'SELECT v.id, v.annee, v.marque, v.modele, v.client_id, u.firstname, u.lastname FROM vehicules v LEFT JOIN users u ON v.client_id = u.id';
...
app.put('/api/vehicules/:id', fieldValidations, (req, _res, next) => {
  ...
  db.query(sql, [annee, marque, modele, client_id, id], (err, result) => {
    if (err) {
      console.error(err);
      res.status(500).send('Server error');
      return;
...
app.post('/api/vehicules', fieldValidations, (req, _res, next) => {
  ...
  const { annee, marque, modele, client_id } = req.body;
  const sql = 'INSERT INTO vehicules (annee, marque, modele, client_id) VALUES (?, ?, ?, ?)';
  db.query(sql, [annee, marque, modele, client_id], (err, result) => {
    if (err) {
      console.error(err);
      res.status(500).send('Server error');
      return;
    }
    res.status(201).send('Vehicule created');
  });
});
